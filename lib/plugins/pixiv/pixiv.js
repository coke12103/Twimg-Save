...
const pixiv = function(url, save_dir){
  ...
  const download = this.twimg_save.download;
  const notification = this.twimg_save.notification;
...
//      }
      ...
      try{
        var image_data = JSON.parse(parse_body.querySelector("#meta-preload-data").attributes.content).illust[parseInt(pixiv_image_id)];
      }catch{
