...
const pixiv = function(url, save_dir){
  ...
  request.get(url).then(async (body) => {
    ...
    if((parse_body.querySelector('.img-container a img') != null) || (parse_body.querySelector('.sensored img'))){
      ...
      for(var script_tag of parse_body.querySelectorAll('script')){
        if(!script_tag.attributes.src){
          if(script_tag.text.match(/use strict/)){
            ...
            var images_len = script_tag.text.match(/"pageCount":([0-9]+)/g);
            images_len = parseInt(images_len[images_len.length -1].match(/[0-9]+/),10);
