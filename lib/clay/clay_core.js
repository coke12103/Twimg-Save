...
class ClayCore{
  ...
  load(plugins_path){
    ...
    for(var l of file_list){
      ...
      if(!(fs.existsSync(l) && fs.statSync(l).isDirectory())) continue;
      ...
      try{
        ...
        if(parse_spell){
          ...
          if(parse_spell.spell_ver){
            ...
            if(parse_spell.type.source_addition) parse_spell.type.source_addition.regexp = new RegExp(parse_spell.type.source_addition.regexp, "i");
          }else{
            // プラグインバージョンなしは削除予定
            // と思ってたんだけど簡易プラグインとして残してもいいのではないか
            //
            if(parse_spell.search_regexp) parse_spell.search_regexp = new RegExp(parse_spell.search_regexp, "i");
    ...
    console.log(plugins_spell_list)
  ...
  find_source(url){
    ...
    for(var spell of spells){
      if(spell.spell_ver){
        if(!spell.type.source_addition) continue;
        if(!spell.type.source_addition.regexp.test(url)) continue;
